...
status_t CameraMetadata::getTagFromName(const char *name,
    ...
    size_t totalSectionCount = EMAI_SECTION_COUNT + vendorSectionCount;
    for (size_t i = 0; i < totalSectionCount; ++i) {
        const char *str = (i < EMAI_SECTION_COUNT) ? camera_metadata_section_names[i] :
                (*vendorSections)[i - EMAI_SECTION_COUNT].string();
