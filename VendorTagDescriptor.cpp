...
status_t VendorTagDescriptor::readFromParcel(const EMAI::Parcel* parcel) {
    status_t res = OK;
    if (parcel == NULL) {
...
status_t VendorTagDescriptor::writeToParcel(EMAI::Parcel* parcel) const {
    status_t res = OK;
    if (parcel == NULL) {
        ALOGE("%s: parcel argument was NULL.", __FUNCTION__);
        return BAD_VALUE;
...
int32_t VendorTagDescriptorCache::getVendorTagDescriptor(
        metadata_vendor_id_t id, sp<EMAI::VendorTagDescriptor> *desc /*out*/) {
    auto entry = mVendorMap.find(id);
    if (entry == mVendorMap.end()) {
